<script setup>
const footerSections = [
  {
    title: '产品',
    links: [
      { name: '产品中心', path: '/products' },
      { name: '个人助手', path: '/products/personal-assistant' },
      { name: '育儿助手', path: '/products/parenting-assistant' },
      { name: 'AI工具助手', path: '/products/ai-tools' },
    ],
  },
  {
    title: '内容',
    links: [
      { name: '昆廷笔记', path: '/columns/kunting' },
      { name: '启鸣宝宝', path: '/columns/qiming' },
      { name: '文章中心', path: '/articles' },
    ],
  },
  {
    title: '互动',
    links: [
      { name: '游戏中心', path: '/games' },
      { name: '快来尝鲜', path: '/try' },
      { name: '实验室', path: '/lab' },
      { name: '心愿单', path: '/wishlist' },
    ],
  },
  {
    title: '关于',
    links: [
      { name: '关于', path: '/about' },
      { name: '加入社区', path: '/community' },
      { name: '隐私政策', path: '/privacy' },
      { name: '网站地图', path: '/sitemap' },
    ],
  },
]
</script>

<template>
  <footer class="bg-brand-charcoal text-white/80 mt-section">
    <div class="container-content py-16">
      <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-8">
        <!-- Brand -->
        <div class="col-span-2 md:col-span-4 lg:col-span-1 mb-4 lg:mb-0">
          <div class="font-bold text-lg text-white mb-3">QUENTIN WINDOW</div>
          <p class="text-sm text-white/50 leading-relaxed">
            独立开发者的窗口<br />产品、内容与实验
          </p>
        </div>

        <!-- Link Sections -->
        <div v-for="section in footerSections" :key="section.title">
          <h4 class="font-medium text-white mb-4 text-sm">{{ section.title }}</h4>
          <ul class="space-y-2.5">
            <li v-for="link in section.links" :key="link.path">
              <router-link
                :to="link.path"
                class="text-sm text-white/50 hover:text-white transition-colors"
              >
                {{ link.name }}
              </router-link>
            </li>
          </ul>
        </div>
      </div>

      <!-- Bottom -->
      <div class="mt-12 pt-8 border-t border-white/10 flex flex-col sm:flex-row justify-between items-center gap-4">
        <p class="text-xs text-white/30">
          &copy; {{ new Date().getFullYear() }} QUENTIN WINDOW. All rights reserved.
        </p>
        <div class="flex gap-4">
          <a href="#" class="text-white/30 hover:text-white/60 transition-colors" aria-label="GitHub">
            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
              <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/>
            </svg>
          </a>
        </div>
      </div>
    </div>
  </footer>
</template>
