import article1 from '@/assets/images/shucai/articles/article-thumb-1.jpg'
import article2 from '@/assets/images/shucai/articles/article-thumb-2.jpg'
import article3 from '@/assets/images/shucai/articles/article-thumb-3.jpg'
import article4 from '@/assets/images/shucai/articles/article-thumb-4.jpg'
import article5 from '@/assets/images/shucai/articles/article-thumb-5.jpg'
import article6 from '@/assets/images/shucai/articles/article-thumb-6.jpg'
import { columns } from './columns'

export const articleCategories = ['全部', 'AI 提效', 'App 开发', '幼小衔接', '亲子游戏', '产品思考']
export const articleHotTopics = ['AI 提效', '独立开发', '育儿启蒙', '亲子游戏', '产品思考', '阅读成长']

export const articles = [
  {
    id: 1,
    slug: 'ai-workflow-output',
    title: '我的 AI 提效工作流：从信息输入到高质量输出',
    summary: '分享我如何把搜集、整理、提炼、写作和复盘串起来，形成稳定可复用的 AI 工作流。',
    readingTime: '8 分钟',
    wordCount: '3686 字',
    likes: 128,
    favorites: 256,
    sections: [
      {
        title: '为什么你需要一个属于自己的效率系统',
        paragraphs: [
          '高质量输出，往往不是从一个更强的模型开始，而是从更清晰的信息输入开始。',
          '很多人在寻找“神奇工具”，但真正稳定的效率，往往来自你是否建立了属于自己的系统。它会让你在不同任务之间切换时，依然能保持清晰和节奏。',
        ],
      },
      {
        title: '第一步：信息收集与处理',
        paragraphs: [
          '我会先把碎片输入分成三类：原始素材、待验证线索、可直接引用的结构化结论。只有这样，后续的 AI 总结才不会越走越散。',
          '信息如果一开始就没有分类，后面你会花更多时间在“重新理解这些资料”上，而不是推进真正重要的工作。',
        ],
      },
      {
        title: '第二步：任务管理与执行',
        paragraphs: [
          '第二步是把问题拆成更小的动作，比如提纲、归纳、改写、补例子，而不是一次把所有工作丢给模型。',
          '拆分动作的好处是，每一步都更容易验证，也更容易让 AI 成为你的协作者，而不是一个不稳定的黑盒。',
        ],
      },
      {
        title: '第三步：回顾与迭代优化',
        paragraphs: [
          '真正提高效率的关键，不是每次都追求神奇输出，而是让同一套流程可以被重复执行。',
          '当你开始定期复盘自己的输入、处理与输出链路时，这套系统才会越用越顺，并逐渐变成你真正的工作资产。',
        ],
      },
    ],
    content: [
      '高质量输出，往往不是从一个更强的模型开始，而是从更清晰的信息输入开始。',
      '我会先把碎片输入分成三类：原始素材、待验证线索、可直接引用的结构化结论。只有这样，后续的 AI 总结才不会越走越散。',
      '第二步是把问题拆成更小的动作，比如提纲、归纳、改写、补例子，而不是一次把所有工作丢给模型。',
      '真正提高效率的关键，不是每次都追求神奇输出，而是让同一套流程可以被重复执行。',
    ],
    cover: article1,
    column: 'kunting',
    category: 'AI 提效',
    tags: ['AI 提效', '工作流', '效率工具'],
    publishedAt: '2026-04-20',
    viewCount: '1.2k',
    author: columns.kunting.author,
    authorAvatar: columns.kunting.avatar,
  },
  {
    id: 2,
    slug: 'kindergarten-transition-rhythm',
    title: '幼小衔接，不焦虑的节奏感养成指南',
    summary: '从作息调整到心理准备，从学习习惯到社交能力，帮助家长和孩子平稳过渡。',
    readingTime: '6 分钟',
    wordCount: '2410 字',
    likes: 96,
    favorites: 188,
    sections: [
      {
        title: '幼小衔接最重要的不是提前学多少',
        paragraphs: [
          '幼小衔接最容易被误解成“提前学很多知识”，但更重要的是节奏感。',
          '比起提前学完课程，更重要的是帮助孩子建立稳定作息、任务意识和面对变化时的安全感。',
        ],
      },
      {
        title: '先把作息与任务感稳定下来',
        paragraphs: [
          '规律作息、任务意识、独立表达和情绪稳定，往往比多做几页练习更有价值。',
          '当孩子知道什么时候该起床、什么时候该收拾、什么时候该完成小任务，很多焦虑会自然下降。',
        ],
      },
      {
        title: '把准备入学变成可持续的小习惯',
        paragraphs: [
          '如果家庭能把“准备入学”变成一套可持续的小习惯，孩子会更从容。',
          '这些习惯不需要很重，但要稳定，比如固定整理书包、简单表达当天感受、自己完成一个小任务。',
        ],
      },
    ],
    content: [
      '幼小衔接最容易被误解成“提前学很多知识”，但更重要的是节奏感。',
      '规律作息、任务意识、独立表达和情绪稳定，往往比多做几页练习更有价值。',
      '如果家庭能把“准备入学”变成一套可持续的小习惯，孩子会更从容。',
    ],
    cover: article2,
    column: 'qiming',
    category: '幼小衔接',
    tags: ['幼小衔接', '育儿启蒙'],
    publishedAt: '2026-04-18',
    viewCount: '856',
    author: columns.qiming.author,
    authorAvatar: columns.qiming.avatar,
  },
  {
    id: 3,
    slug: 'first-app-in-90-days',
    title: '独立开发者的第一个 App：从 0 到 1 的 90 天',
    summary: '记录从想法萌芽到 App 上架的完整过程，包括技术选型、产品设计和上线后的反馈。',
    readingTime: '9 分钟',
    wordCount: '4026 字',
    likes: 173,
    favorites: 221,
    sections: [
      {
        title: '先别急着做，先确定第一步',
        paragraphs: [
          '从 0 到 1 的困难，很多时候不是“不会做”，而是“不知道先做哪一步”。',
          '如果一开始就试图同时解决技术、设计、运营和增长，你很快就会在复杂度里失速。',
        ],
      },
      {
        title: '把 90 天拆成 5 个阶段',
        paragraphs: [
          '我把 90 天拆成调研、原型、 MVP、验证和上线五个阶段，每个阶段只抓一个主目标。',
          '这样做最大的价值不是“更专业”，而是让你始终知道当前阶段最重要的事情是什么。',
        ],
      },
      {
        title: '用持续曝光代替一次性冲刺',
        paragraphs: [
          '最有效的节奏不是连续高强度冲刺，而是每周都能对外看到一点进展。',
          '持续曝光会带来外部反馈，也会让自己更难逃避那些真正影响上线的问题。',
        ],
      },
    ],
    content: [
      '从 0 到 1 的困难，很多时候不是“不会做”，而是“不知道先做哪一步”。',
      '我把 90 天拆成调研、原型、 MVP、验证和上线五个阶段，每个阶段只抓一个主目标。',
      '最有效的节奏不是连续高强度冲刺，而是每周都能对外看到一点进展。',
    ],
    cover: article3,
    column: 'kunting',
    category: 'App 开发',
    tags: ['独立开发', 'App 开发', '产品思考'],
    publishedAt: '2026-04-15',
    viewCount: '1.6k',
    author: columns.kunting.author,
    authorAvatar: columns.kunting.avatar,
  },
  {
    id: 4,
    slug: 'family-boardgame-list',
    title: '亲子桌游推荐：让陪伴变得更有趣',
    summary: '精选适合 3-8 岁孩子的亲子桌游，从益智启蒙到策略思维，附带年龄适配建议。',
    readingTime: '7 分钟',
    wordCount: '2760 字',
    likes: 88,
    favorites: 164,
    sections: [
      {
        title: '什么样的桌游更适合亲子陪伴',
        paragraphs: [
          '真正适合亲子陪伴的桌游，不一定是规则最复杂的，而是能让孩子愿意反复参与的。',
          '孩子需要的是容易进入、反馈及时、不会总是失败的体验，而不是成人视角里的“高级玩法”。',
        ],
      },
      {
        title: '优先考虑反馈快、挫败感低的游戏',
        paragraphs: [
          '我会优先挑选反馈快、挫败感低、家长也愿意一起玩的桌游。',
          '这样的游戏更容易形成高质量陪伴，而不是把亲子时间变成新的任务或压力来源。',
        ],
      },
      {
        title: '陪伴价值往往大于知识点',
        paragraphs: [
          '如果一款游戏能自然带出轮流、观察、表达和情绪管理，它的价值会远大于“学到了什么知识”。',
          '真正值得反复玩的桌游，通常是孩子愿意主动拿出来，家长也不觉得消耗的那种。',
        ],
      },
    ],
    content: [
      '真正适合亲子陪伴的桌游，不一定是规则最复杂的，而是能让孩子愿意反复参与的。',
      '我会优先挑选反馈快、挫败感低、家长也愿意一起玩的桌游。',
      '如果一款游戏能自然带出轮流、观察、表达和情绪管理，它的价值会远大于“学到了什么知识”。',
    ],
    cover: article4,
    column: 'qiming',
    category: '亲子游戏',
    tags: ['亲子游戏', '陪伴成长'],
    publishedAt: '2026-04-12',
    viewCount: '732',
    author: columns.qiming.author,
    authorAvatar: columns.qiming.avatar,
  },
  {
    id: 5,
    slug: 'shipping-small-products-fast',
    title: '小产品快速上线的三个原则',
    summary: '减少功能幻想、尽快触达真实用户、让反馈驱动迭代，是小产品生存的关键。',
    readingTime: '5 分钟',
    wordCount: '1880 字',
    likes: 104,
    favorites: 149,
    sections: [
      {
        title: '小产品最大的风险不是做得少',
        paragraphs: [
          '小产品最大的风险不是功能少，而是做得太慢。',
          '当你为了“更完整”不断往后延期时，很可能已经错过了验证方向最好的时机。',
        ],
      },
      {
        title: '让用户更早看到真实进展',
        paragraphs: [
          '如果一件事情三周后才能让用户看到，你很可能已经在错误方向上投入太多。',
          '小产品更需要尽快暴露在真实反馈中，而不是在想象里打磨一个“完美版本”。',
        ],
      },
      {
        title: '把迭代建立在反馈而不是幻想上',
        paragraphs: [
          '上线不是终点，而是让产品进入真实世界的起点。你越早和用户对话，越能知道哪些能力值得继续投入。',
          '反馈驱动的迭代，才是小产品持续活下去的关键机制。',
        ],
      },
    ],
    content: [
      '小产品最大的风险不是功能少，而是做得太慢。',
      '如果一件事情三周后才能让用户看到，你很可能已经在错误方向上投入太多。',
    ],
    cover: article5,
    column: 'kunting',
    category: '产品思考',
    tags: ['产品思考', '独立开发'],
    publishedAt: '2026-04-09',
    viewCount: '967',
    author: columns.kunting.author,
    authorAvatar: columns.kunting.avatar,
  },
  {
    id: 6,
    slug: 'pinyin-learning-at-home',
    title: '在家学拼音，怎么把练习变成愿意参与的游戏',
    summary: '与其追求每天练很多，不如把一次次短时、有效、愿意继续做的体验建立起来。',
    readingTime: '5 分钟',
    wordCount: '1965 字',
    likes: 76,
    favorites: 137,
    sections: [
      {
        title: '孩子最怕的不是拼音本身',
        paragraphs: [
          '拼音启蒙最怕的是孩子只记住“好难”。',
          '一旦孩子把拼音和挫败感绑定在一起，后面的练习就会越来越抗拒。',
        ],
      },
      {
        title: '缩短节奏，让反馈更快',
        paragraphs: [
          '把节奏拆短、反馈变快、挑战变轻，孩子更容易愿意重复。',
          '与其一次练很久，不如每次都留下“我还能再来一次”的感觉。',
        ],
      },
      {
        title: '把练习设计成可参与的游戏',
        paragraphs: [
          '如果把拼音练习做成轻量闯关、角色反馈或小目标达成，孩子会更容易主动参与。',
          '真正有效的练习，不是压着孩子学，而是让孩子愿意继续玩下去。',
        ],
      },
    ],
    content: [
      '拼音启蒙最怕的是孩子只记住“好难”。',
      '把节奏拆短、反馈变快、挑战变轻，孩子更容易愿意重复。',
    ],
    cover: article6,
    column: 'qiming',
    category: '育儿启蒙',
    tags: ['拼音启蒙', '幼小衔接'],
    publishedAt: '2026-04-06',
    viewCount: '648',
    author: columns.qiming.author,
    authorAvatar: columns.qiming.avatar,
  },
]

export const blogPosts = articles.slice(0, 4)

export const getArticleBySlug = (slug) => articles.find((item) => item.slug === slug)
